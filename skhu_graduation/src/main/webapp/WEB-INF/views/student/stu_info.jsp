		...
		<div id="jb-header">
			<div id='cssmenu'>
				<ul>
					...
					<li><a href='stu_main'><span>나의졸업요건</span></a></li>
					...
					<li style="float: right"><a href='redirect:stu_Info'><span>개인정보변경</span></a></li>
				</ul>
			</div>
		</div>
							...
							<div class="form-group">
								...
								<div class="cols-sm-10">
									<div class="input-group">
										...
										&nbsp;&nbsp;
										<input type="password" class="form-control" name="password" data-toggle="popover" placeholder="비밀번호 입력" style="height: 37px;"/>
									</div>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="cols-sm-10">
									<div class="input-group">
										<span class="input-group-addon"><i class="fa fa-lock" aria-hidden="true"  style="margin-top: 8px;"></i></span>
										&nbsp;&nbsp;
										<input type="password" class="form-control" name="confirmPassword" placeholder="비밀번호 확인 입력" style="height: 37px;"/>
									</div>
								</div>
							</div>
                  			...
                  			<div class="form-group" style="margin-top: 20pt;">   
                     			<div class="cols-sm-10">
                        			...
                        			<div class="input-group" style="font-size: 13pt;">
                        				<span class="input-group-addon"><i class="fas fa-pen-square" ></i>&nbsp; 편입생</span>
                       				...
                       				<div class="input-group" style="font-size: 13pt; margin-top: 8px;">
                           				<span class="input-group-addon"><i class="fas fa-pen-square" ></i>&nbsp; 사회봉사면제여부</span>
                  			...
                  			<div class="form-group" style="margin-top: 25px;">
                     			...
                     			<div class="radio" style="font-size: 10pt; margin-bottom: 7%">
                         			&nbsp;&nbsp;&nbsp;&nbsp;
                         			<input type="radio" name="howToGraduate" value="전공기초" ${ student.howToGraduate == '전공기초' ? 'checked' : '' } />전공기초 
                         			&nbsp; &nbsp;
                        			<input type="radio" name="howToGraduate"value="전공심화" ${ student.howToGraduate == '전공심화' ? 'checked' : '' } /> 전공심화
                      			</div>
                     			...
                     			<div class="cols-sm-10">
                        			<div class="input-group" style="margin-bottom: 7%">
                           				...
                           				<select name="departmentId" class="form-control" style="height: 37px; font-size: 10pt;">
                                 			<option value="0" ${student.departmentId == 0 ? 'selected' : '' }>주전공을 선택해 주세요</option>
                                 			<option value="12" ${student.departmentId == 12 ? 'selected' : ''}>소프트웨어 공학과</option>
                                 			<option value="14" ${student.departmentId == 14 ? 'selected' : ''}>컴퓨터공학과</option>
                                 			<option value="10" ${student.departmentId == 10 ? 'selected' : ''}>IT융합 자율 학부</option>
                           				</select>
                        			</div>
                     			</div>
                     			...
                     			<div class="cols-sm-10">
									<div class="input-group" style="margin-bottom: 7%">
										...
										<select name="stuClass" class="form-control" style="height: 37px; font-size: 10pt;">
											<option value="0">반을 선택해주세요</option>
											<option value="a" ${student.stuClass == 'a' ? 'selected' : '' }>a반</option>
											<option value="b" ${student.stuClass == 'b' ? 'selected' : '' }>b반</option>
											<option value="c" ${student.stuClass == 'c' ? 'selected' : '' }>c반</option>
											<option value="d" ${student.stuClass == 'd' ? 'selected' : '' }>d반</option>
										</select>
									</div>
								</div>
                     		...
                     		<div class="checkbox" style="font-size: 10pt; margin-bottom: 7%">
                         		&nbsp;&nbsp;&nbsp;&nbsp; 
                         		<input type="checkbox" name="division" value="복수전공" ${ secondMajor.division == '복수전공' ? 'checked' : ''} /> 복수전공 
                         		&nbsp; &nbsp;
                        		<input type="checkbox" name="division" value="부전공" ${ secondMajor.division == '부전공' ? 'checked' : ''} /> 부전공
                     		</div>
                           				...
                           				<select name="stuSemester" class="form-control" style="height: 37px; font-size: 10pt;" >
                                			...
                                			<option value="2" <c:if test="${ student.stuSemester == 2 }">selected</c:if>>2학기</option>
                                 			...
                                 			<option value="5" <c:if test="${ student.stuSemester == 5 }">selected</c:if>>5학기</option>
                                  			<option value="6" <c:if test="${ student.stuSemester == 6 }">selected</c:if>>6학기</option>
                                 			<option value="7" <c:if test="${ student.stuSemester == 7 }">selected</c:if>>7학기</option>
                                 			<option value="8" <c:if test="${ student.stuSemester == 8 }">selected</c:if>>8학기</option>
                        				</select>
                        			</div>
                     			</div>
                  			</div>
